    ...
    def run_simulation(
        ...
        def find_turns(trajectory: Trajectory) -> np.ndarray:
            ...
            for i in range(len(trajectory)):
                last_curvature = trajectory[trajectory.dec(i), Trajectory.CURVATURE]
                this_curvature = trajectory[i, Trajectory.CURVATURE]
                next_curvature = trajectory[trajectory.inc(i), Trajectory.CURVATURE]
        ...
        for turn in turns:
            ...
            turn_pt[Trajectory.SPEED] = min(
                self.calc_v(
                    self.vehicle.lookup_acc_circle(lon=0.0),
                    turn_pt[Trajectory.CURVATURE],
                ),
                self.vehicle.max_speed_mps,
            )
        ...
        while True:
            ...
            for flags in iteration_flags:
                ...
                max_state_speed = sqrt(
                    2 * max_acc * dd + last_enter_pt[Trajectory.SPEED] ** 2
                )
                min_state_speed = sqrt(
                    2 * min_acc * dd + last_enter_pt[Trajectory.SPEED] ** 2
                )
